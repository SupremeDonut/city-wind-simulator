...
wind_sim/
├── assets/
│   └── presets/               # Pre-computed city datasets
│       ├── chicago.glb        # 3D building mesh
│       ├── chicago.h5         # Voxel occupancy grid (HDF5)
│       ├── manhattan.glb
│       ├── manhattan.h5
│       ├── san_francisco.glb
│       └── san_francisco.h5
...
│   ├── simulation.py          # Analytical mock fallback + preset registry
│   ├── results/               # Job output .npz files (gitignored)
│   └── routes/
│       ├── presets.py         # GET /presets, GET /presets/{id}/geometry
│       ├── simulate.py        # POST /simulate, WS /ws/simulation/{id}
│       ├── predict.py         # POST /predict (analytical fallback)
│       ├── results.py         # GET /results/{id}/...
│       └── map_texture.py     # GET /map-texture/{id}
...
│   ├── package.json
│   └── vite.config.ts
├── geometry/                  # Offline preset generation
│   ├── overture_mesh.py       # Overture Maps → GLB + HDF5 occupancy grid
│   └── osm_tiles.py           # OSM tile fetching for map textures
├── simulation/
│   ├── lbm_py/                # Python lbm package (uv workspace member)
│   │   ├── lbm/
│   │   │   ├── __init__.py
│   │   │   ├── run_cuda.py    # CUDA solver subprocess wrapper (API entry point)
│   │   │   └── preprocess.py  # Voxel rotation + downsampling
│   │   ├── dev/               # Reference implementations (not in pipeline)
│   │   │   ├── d2q9.py        # 2D LBM proof-of-concept
│   │   │   ├── d3q19.py       # 3D Python LBM reference implementation
│   │   │   ├── benchmark.py   # CUDA vs Python performance comparison
│   │   │   └── visualize.py   # PyVista 3D field viewer
│   │   └── pyproject.toml
│   └── src/                   # CUDA C++ solver
│       ├── main.cu
│       ├── lbm_kernels.cu
│       ├── lbm_kernels.cuh
│       ├── lattice_constants.cuh
│       └── CMakeLists.txt
├── pyproject.toml             # uv workspace root (api, geometry, lbm_py)
└── .gitignore
```
